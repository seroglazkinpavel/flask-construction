...
- RegistrationForm - форма регистрации, имеет четыре поля: username, email, password, confirm_password. Поле username проверяется на наличие данных и на минимальную, максимальную длину (min=3, max=25). Поле email проверяется на наличие данных и на соответствие формату email. Поле password проверяется на наличие данных и на минимальную длину (6 символов). Поле confirm_password проверяется на наличие данных и на соответствие значению поля password. Созданный класс RegistrationForm импортируем в app.py и будем использовать его в функции представления register(). Сначала убеждаемся, что пользователь не вошел в систему. Схема работы такая же, как и для входа в систему. Логика, выполняемая внутри функции register() такая, если метод POST получаем данные от пользователя, сравниваем их с данными в базе данных и деланм вывод, если таких данных нет то, создаем нового пользователя с именем, электронной почтой и паролем, записывает их в базу данных и затем перенаправляет запрос на вход login.html, чтобы пользователь мог войти в систему, иначе делаем редирект на страницу регистрации.
- CommentForm - форма комментариев, имеет два поля: text, submit. Каждое поле ссылаются на соответствующие объекты: TextAreaField, SubmitField, которые импортируем из wtforms в файл forms.py. Созданный класс CommentForm импортируем в app.py и будем использовать его в функции представления reviews(). В нутри функции создаем экземпляр класса CommentForm. Логика, выполнения внутри условия if form.validate_on_submit(): создает новый комментарий и записывает его в базу данных. Дальше выводятся комментарии с учетом пагинации.
- AdminForm - форма удаления комментария, имеет одно поле: submit, которое ссылается на объект SubmitField. Созданный класс AdminForm импортируем в app.py и будем использовать его в функции представления delete_comment(comment_id). В нутри функции создаем экземпляр класса AdminForm. Если в функцию передаем несуществующий идентификатор,то выводим страницу 404, иначе комментарий удаляем.
- ReviewsForm - форма обновления комментария, имеет четыре поля: id, author, text, submit. Каждое поле ссылаются на соответствующие объекты: IntegerField, StringField, TextAreaField, SubmitField. . Созданный класс ReviewsForm импортируем в app.py и будем использовать его в функции представления edit_comment(comment_id). В нутри функции создаем экземпляр класса ReviewsForm. Логика, выполнения внутри условия if form.validate_on_submit(): позволяет обновить автора, текст.

Запуск приложения осуществляется в файле wsgi.py. В стадии разработки укажим app.run(debug=True), для того, чтобы видеть все ошибки при создании проекта.

Стек:: Python, MySQL,фреймворк Flask, SQLAlchemy, WTForm, Jinga2, Javascript, JQuery, библиотека Flask-Login, css, html
